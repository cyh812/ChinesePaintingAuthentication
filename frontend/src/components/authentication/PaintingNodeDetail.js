...
const PaintingNodeDetail = ({ node, onPaintingSegmentSearchResult }) => {
  const paintingId = node?.id || "";
  const paintingTitle = node?.name || "未命名画作";

  const [searchSignal, setSearchSignal] = useState(0);
  const [selectedSegmentIndex, setSelectedSegmentIndex] = useState(null);

  // 用 id 和 “编号” 对齐查找
  const paintingRecord = PaintingData.find((item) => item["编号"] === paintingId);

  const authorName = paintingRecord?.["作者名"] || "未知";
  const creationTime = paintingRecord?.["创作时间"] || "未知";
  const material = paintingRecord?.["用色"] || "未知";
  const size = paintingRecord?.["尺寸"] || "未知";
  ...
  const segmentList = useMemo(() => {
    const currentPaintingSegments = paintingsegments?.[paintingId];
  ...
  const getPaintingNameById = (id) => {
    const record = PaintingData.find((item) => item["编号"] === id);
    return record?.["总作品名"] || record?.["作品名"] || record?.["编号"] || id;
  };
      ...
      <div className="painting-node-detail-info-list">
        <div className="painting-node-detail-info-item painting-node-detail-info-row">
          <div className="painting-node-detail-inline-group">
            <span className="painting-node-detail-label">作者</span>
            <span className="painting-node-detail-value">{authorName}</span>
          </div>

          <div className="painting-node-detail-inline-group">
            <span className="painting-node-detail-label">年代</span>
        ...
        <div className="painting-node-detail-info-item painting-node-detail-info-row">
          <div className="painting-node-detail-inline-group">
            <span className="painting-node-detail-label">材质</span>
            <span className="painting-node-detail-value">{material}</span>
          </div>

          <div className="painting-node-detail-inline-group">
            <span className="painting-node-detail-label">尺寸</span>
